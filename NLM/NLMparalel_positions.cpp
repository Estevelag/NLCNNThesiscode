...
{                                // Time variables
    ...
    int Totalwidth = 84; // because it's expanded to get the original image in the algorithm, it should be 84
    int window_size = 21;
    int patch_size = 7;
    // vector to save the distances
    std::vector<double> dist((Totalwidth - window_size) * (Totalwidth - window_size) * 20 * 2); // This is all he values it should save (64*64*20*2) in the image proposed
    ...
    for (int ke = 0; ke < Totalwidth - window_size; ke++)
    {
        for (int me = 0; me < Totalwidth - window_size; me++) // iterate over all posible windows
        {
            ...
            for (int ve = ke; ve < ke + window_size - patch_size; ve++)
            {
                ...
                for (int we = me; we < me + window_size - patch_size; we++)
                {
                    ...
                    if (Muestras < 20)
                    {
                        ...
                        {
                            dist[40 * (ke) + (Totalwidth - window_size) * 40 * (me) + 2 * Muestras] = ve + patch_size / 2;
                            dist[40 * (ke) + (Totalwidth - window_size) * 40 * (me) + 2 * Muestras + 1] = we + patch_size / 2;
                            Muestras = Muestras + 1;
            ...
            if (Muestras < 20)
            {
                while (Muestras < 20)
                {
                    dist[40 * (ke) + (Totalwidth - window_size) * 40 * (me) + 2 * Muestras] = ke + window_size / 2;
                    dist[40 * (ke) + (Totalwidth - window_size) * 40 * (me) + 2 * Muestras + 1] = me + window_size / 2;
                    Muestras = Muestras + 1;
                }
            }
